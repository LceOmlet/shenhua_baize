...
"""
    ...
    def process(self, audio_path: str) -> ExtractionResult:
        """完整处理流程"""
        try:
            # 1. 格式转换
            ...
            confidence = whisper_result.get("avg_logprob", 0.0)
            ...
            return ExtractionResult(
                content_type="audio",
                original_data=audio_path,
                extracted_fields={"error": f"JSON解析失败: {str(e)}"},
                confidence=0.0
            )
        except Exception as e:
            return ExtractionResult(
                content_type="audio",
                original_data=audio_path,
                extracted_fields={"error": f"处理失败: {str(e)}"},
                confidence=0.0
            )
